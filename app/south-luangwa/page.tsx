...
export default function SouthLuangwaPage() {
  ...
  return <main className="subpage">
    ...
    <section className="section gallery-page-section"><div className="shell"><div className="section-heading"><p className="eyebrow">Gallery</p><h2>South Luangwa landscapes</h2></div><div className="south-gallery-grid">{wildlife.map((item) => <figure className="south-gallery-card" key={item.src}><img src={item.src} alt={item.alt} /><figcaption>{item.label}</figcaption></figure>)}</div><div className="section-footer-link"><Link href="/gallery" className="button button--forest">View All <span>&#8599;</span></Link></div></div></section>
  </main>;
}
